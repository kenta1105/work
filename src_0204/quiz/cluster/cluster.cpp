...
void clusterHelper(const std::vector<float>> points, std::vector<int>& cluster, KdTree* tree, float distanceTol, int index, std::vector<bool>& checked)
{
	checked[index] = true;
	...
	for (int nearID : nearest)
	{
		if (!checked[nearID])
		{
			clusterHelper(points, cluster, tree, distanceTol, nearID, checked);
		}
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for (int index = 0; index < points.size(); index++)
	{
		if (!checked[index])
		{
			...
			clusterHelper(points, cluseter, tree, distanceTol, index, checked);
