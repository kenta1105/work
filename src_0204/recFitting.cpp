...
float recFitting::calc_closeness_criterion(Eigen::MatrixXf& cluster_2d_vc)
{
    ...
    Eigen::VectorXf D1 = a.cwiseMin(b); // 各要素の最小値をとる
    ...
    Eigen::VectorXf D2 = c.cwiseMin(d); // 各要素の最小値をとる

    Eigen::VectorXf minDis = min_dist_of_closeness_crit * Eigen::VectorXf::Ones(cluster_2d_vc.cols()); // 距離閾値の行列

    Eigen::VectorXf D3 = D1.cwiseMin(D2); // 各要素の最小値をとる
    Eigen::VectorXf D4 = D3.cwiseMax(minDis); // 各要素の最大値をとる
    Eigen::VectorXf D5 = D4.cwiseInverse(); // 分の1に変換
    float distance = D5.sum(); // 合計
    return distance;
}
...
int recFitting::calc_boundingBox(float minp[2], Eigen::MatrixXf& cluster_3d)
{
    ...
    minTrans << cosf(minp[1]), sinf(minp[1]), -sinf(minp[1]), cosf(minp[1]); // 回転行列
...
{
    float dtheta = dtheta_deg_for_serarch * (M_PI / 180.0F); // 分解能をdeg⇒radに変換
    float minp[2] = {-FLT_MAX, 0.0};                         // コスト,角度

    // x,y,z to x,y
    Eigen::MatrixXf cluster_2d = cluster_3d.topRows(2);      // 2行に変換
...
void recFitting::fitting(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloudPoints, std::vector<pcl::PointIndices>& cluster_inds)
{
    ...
    {
        Eigen::MatrixXf cluster_3d(3, it.indices.size()); // 3×クラスターごとのサイズの行列
        int index = 0;
        ...
        {
            // 0行目：x, 1行目:y,2行目：zを行列に格納(列ごとに点群切り替え)
